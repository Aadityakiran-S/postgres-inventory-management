...
const findMinPriceBtweenTwoDates = async (req, res) => {
    let { productName, supplierName, customerID, date1, date2 } = req.body;
    try {
        ...
        if (!queryResult2.rows[0].exists) {
            return res.status(404).json({ success: false, message: `Supplier with name: ${supplierName} DNE` });
        }
        //#endregion

        let query3 = {
            name: `Return min within date range`,
            text: `SELECT * FROM invoice_item WHERE supplier_id = $1 AND product_id = $2 AND customer_id = $3 AND date BETWEEN $4 AND $5 ORDER BY unit_price ASC LIMIT 1;`,
            values: [supplierID, productID, customerID, date1, date2]
        }
